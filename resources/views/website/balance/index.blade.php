                    ...
                    <div class="col  p-0 text-center">
                        <div class="mx-0 mb-3">
                            ...
                            @else
                                <h3 class="color-gray2-dark font-16 mb-0">
                                    @if($order->getDiscount() > 0)
                                <span class="old-price">{{$order->getDiscount() + $order->fees }}</span>
                            @endif
                            {{$order->fees}} ريال</h3>
                            @endif
                        </div>
...
{{--                    @endif--}}
                ...
                @if(Auth::user()->available_balance >= $order->total_amount)
                <div class="mt-2">
                    @if($type == 'public_order')
                    <a href="{{route('balance.public_order_payment_from_balance',['id'=>$order->id])}}" class="btn btn-m w-100 btn-full rounded-l bg-green-c mr-auto ml-auto font-900 text mt-4">ادفع من الرصيد</a>
                    @else
