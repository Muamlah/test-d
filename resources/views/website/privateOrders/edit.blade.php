    ...
    <div class="page-content header-clear-medium">
        <div class="card card-style bg-grey-c card-order" data-card-height="130">
            <div class="card-center text-center mr-3">
                <h1 class="color-white mb-0"> تعديل طلب تعميد خاص رقم {{$order->id}}</h1>
            </div>
        </div>
        ...
        @if (Session::get('edit_error') )
            <div class="ml-3 mr-3 alert alert-small rounded-s shadow-xl bg-red2-dark" role="alert">
                <span><i class="fa fa-check"></i></span>
                <strong>{{Session::get('edit_error')}}</strong>
                <button type="button" class="close color-white opacity-60 font-16" data-dismiss="alert" aria-label="Close">&times;</button>
            </div>
        @endif
...
{{--                    <a href="#" data-tab-active data-tab="tab-private">طلب تعميد خاص</a>--}}
                ...
                <div class="pt-2">
                    <form  action="{{route('privateOrder.update',$order->id)}}" method="POST" class="form-horizontal" role="form">
                        @method('PUT')
                        @csrf
                        <div class="row mb-1">
                            <div class="col-12">
                                <div class="input-style input-style-2 has-icon input-required">
                                    ...
                                    <input id="basicFlatpickr-2" class="form-control flatpickr text-right color-trn flatpickr-input max-date" value="{{\Carbon\Carbon::parse($order->deadline)->format('Y-m-d')}}" type="text" name="date" required    >
                                </div>
                            </div>
                            {{-- <div class="col-6">
                                <div class="input-style input-style-2 has-icon input-required">
                                    ...
                                    <input id="timeFlatpickr"
                                           ...
                                           value="{{\Carbon\Carbon::parse($order->deadline)->format('H:i')}}"
...
@section('script')
                <script type="text/javascript">
                    var f1 = flatpickr(document.getElementById('basicFlatpickr-2'), {
