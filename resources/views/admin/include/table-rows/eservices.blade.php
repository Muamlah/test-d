@foreach($eservices_orders as $order)
    <tr class="h-40px">
        <th scope="row">
            <a href="{{route('admin.eservices_orders.edit', $order->id)}}" target="_blank">
                {{$order->id}}
            </a>
        </th>
        <td>
        ...
        <td><a href="/admin/chat/eservices/{{$order->id}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="محادثة">
                <i class="fab fa-whatsapp"></i>
            </a></td>
    </tr>
@endforeach
